...
from mlworkflow import svm, abc, bgc, bnl, etc, mlp, rfc, xgb
import numpy as np
...
class ModelTest(unittest.TestCase):
    ...
    def test_experiment_01(self):
        ...
        df[list(drugs)] = df[list(drugs)].fillna(0)
        ...
        for selector in selected_vars:
            ...
            for fold, (train_index, test_index) in enumerate(kfold.split(x, y)):
                ...
                for fit_predict in [svm, abc, bgc, bnl, etc, mlp, rfc, xgb]:

                    y_hat = fit_predict(x_train, y_train, x_test)
                    ...
                    result = result.append(pd.DataFrame({
                        'model': [fit_predict.__name__], 'selector': [selector],
                        'fold': [fold + 1], 'auc': [auc], 'acc': [acc],
                        'tp': [tp], 'tn': [tn], 'fp': [fp], 'fn': [fn],
                        'n_vars': [n_vars], 'vars': [','.join(list(selected_vars[selector]))]}))

                    result.to_csv('../output/experiment_one_all_models.tsv', sep='\t', index=False)
