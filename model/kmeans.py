...
class OptimizedKMeans(object):
    ...
    def __init__(self, min_n_clusters=2, max_n_clusters=None, min_n_observations=1,
                 early_stopping_rounds=5, random_state=None, verbose=0, optimization_output_path=None):
        
        assert isinstance(min_n_clusters, int) and min_n_clusters > 1, \
            "min_n_cluster should be an integer greater than one"
        ...
        assert verbose is None or (isinstance(verbose, int) and verbose >= 0), \
            "verbose should be integer greater than or equals to zero"
        ...
        while True:
            ...
            try:
                if np.min(c_sizes) >= self.min_n_observations:
                    opt_metric = (np.mean(ith_cluster_silhouette_values) -
                                  np.std(ith_cluster_silhouette_values)) / sd_sizes;
                else:
                    opt_metric = 0.0
