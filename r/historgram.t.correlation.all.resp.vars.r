...
mic <- read.csv('data/output/correlation_t_x_response_vars_mic.csv', stringsAsFactors = FALSE)
mic[is.na(mic)] <- 0.0
mic['METHOD'] <- 'Maximal Information Coefficient'
colnames(mic) <- c('IndependentVariable', 'DependentVariable', 'Value', 'Method')

dis <- read.csv('data/output/correlation_t_x_response_vars_distcorr.csv', stringsAsFactors = FALSE)
dis[is.na(dis)] <- 0.0
dis['METHOD'] <- 'Distance Correlation'
colnames(dis) <- c('IndependentVariable', 'DependentVariable', 'Value', 'Method')

pea <- read.csv('data/output/correlation_t_x_response_vars_pearson.csv', stringsAsFactors = FALSE)
pea[is.na(pea)] <- 0.0
...
p <- 
  ggplot(df) + 
  geom_bar(stat='identity', aes(x=IndependentVariable, y=abs(Value))) + 
  coord_flip() +
...
ggsave('images/histogram_t_correlation_all_dep_vars.pdf', p, units='cm', width=16, height=24)
